...
public class EnveloppeConvexe {
	...
	public LinkedList<Point> getEnveloppeConnexe() {
		return EC;
	}
	
	/*
	 * Coté d'un point p par rapport aux 2 premier par calcul du
	 * déterminant si l'angle (p0p1,p0p) est direct retour 1 
	 * s'il est indirect retour -1,
	 * s'il est un multiple de pie, retourner 0
	 */
	
	public byte cote(Point p0, Point p1, Point p) {
	...
	private void findHull(Point[] points, Point A, Point B) {
		if (points.length == 0) {
			...
			EC.add(C);
			byte cote1 = cote(C, A, B);
			byte cote2 = cote(C, B, A);
	...
	/*
	 * Fusion qui permet de relier les 2 sous enveloppes convexes
	 * par détermination des points de la tangente la plus haute
	 * et ceux de la tangente la plus basse
	 */
	private Point[] fusion(Point[] ECG, Point[] ECD) {
		if (ECG == null && ECD == null) {
			...
			while(!ok) {
				...
				while(gh>0) {
					if(cote(ECGy[dh], ECDy[gh], ECDy[gh-1])< 0){
						gh--;
						ok = true;
					}
					else {break;}
			...
			while(!ok) {
				ok = false;
				while(db < ECDy.length-1){
					if(cote(ECGy[gb], ECGy[db], ECGy[db + 1]) < 0){
						db++;
					}
					else {break;}
				}
				while(gb < ECGy.length-1) {
			...
			/*
			 * Récupération des éléments de l'enveloppe de gauche
			 * qui seront dans l'enveloppe final suivant:
			 * Ph point le plus haut, Pb: Point le plus bas, P point quelconque
			 * l'angle PhPb, PhP doit être dans le sens des aiguille d'une montre 
			 */
			for(Point p: ECGy) {
			...
			/*
			 * Récupération des éléments de l'enveloppe de droite
			 * qui seront dans l'enveloppe final suivant:
			 * Ph point le plus haut, Pb: Point le plus bas, P point quelconque
			 * l'angle (PhPb, PhP) doit être dans le sens direct
			 */
			for(int i=ECDy.length-1; i>=0; i--) {
				if(cote(ECGy[dh], ECGy[db], ECDy[i]) >= 0) {
